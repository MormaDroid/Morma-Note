...
class DatastoreVM @Inject constructor(private val datastore: DatastoreRepository) : ViewModel() {
    ...
    fun getImgUri() {
        viewModelScope.launch(Dispatchers.IO) { imgUri.value = (datastore.getString(AUTH_IMG_URI_KEY)) }
    }
    ...
    val appColor = MutableStateFlow<Int>(DarkBlue.toArgb())
    fun getAppColor(){ viewModelScope.launch(Dispatchers.IO) {
            datastore.getInt(APP_THEME)?.let{
                appColor.value = it
            }
        } }

    val appFont = MutableStateFlow<Int>(ysabeauMedium)
    fun getAppFont(){
        viewModelScope.launch(Dispatchers.IO) {
            datastore.getInt(APP_FONT)?.let{ appFont.value = it }
        }
    }
